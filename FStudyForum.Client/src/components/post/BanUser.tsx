...
const BanUser: React.FC<BanUserProps> = ({ post, topic, onClose }) => {
  ...
  return (
    <div
      ...
      className={cn(
        "fixed inset-0 z-[999] grid h-screen w-screen place-items-center bg-black bg-opacity-60 opacity-100 backdrop-blur-sm transition-opacity duration-300"
      )}
      onClick={onClose}
    >
      <div
        data-dialog="dialog"
        className={cn(
          "relative m-4 w-2/5 min-w-[50%] max-w-[70%] rounded-lg bg-white font-sans text-base font-light leading-relaxed text-blue-gray-500 antialiased shadow-2xl"
        )}
